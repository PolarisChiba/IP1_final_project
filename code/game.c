...
int player_num;
int h_sz = 0;
int history_score[100];
ALLEGRO_SAMPLE* bgmg;
int ddd = 0;
ALLEGRO_SAMPLE *button_s;
ALLEGRO_SAMPLE_ID button_id;
...
static const char* game_title = "I2P(I)_2020 Final Project <109062309>";
...
void game_create() {
	{
		...
		//fprintf(fp, "nosdf");
		while (fscanf(fp, "%d ", &history_score[h_sz]) != EOF)
			h_sz += 1;
		fclose(fp);
	}
    ...
    game_log("Game begin");
    button_s = load_audio("./bgm/button.ogg");
...
static void game_start_event_loop(void) {
    ...
    while (!done) {
        ...
        if (event.type == ALLEGRO_EVENT_DISPLAY_CLOSE) {
			...
			if (active_scene.name[0] == 'M' && active_scene.name[1] == 'e' && event.mouse.button == 1 && pnt_in_rect(event.mouse.x, event.mouse.y, WINDOW_W - 420, 750, 250, 125)) {
				done = true;
			}
